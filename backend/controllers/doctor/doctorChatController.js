...
export async function handleDoctorChat(req, res) {
  try {
    ...
    const bpData = await BPTracking.findOne({ userId: patientId }).lean();
    const sugarData = await SugarTracking.findOne({ userId: patientId }).lean();
    const reminders = await Reminder.find({ userId: patientId }).sort({ reminderDateTime: -1 }).lean();
...
${reminders.slice(0, 5).map((r, i) =>
  `${i+1}. ${r.title} - ${r.description} (${new Date(r.reminderDateTime).toLocaleDateString()}) [${r.status}]`
).join('\n')}
=== END REMINDERS ===
` : "No reminders set.\n";
