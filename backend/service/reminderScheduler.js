...
export const initializeScheduler = () => {
  ...
  // Run once on boot so due reminders aren't missed while waiting for the first cron tick
  setTimeout(() => {
  ...
  const checkJob = cron.schedule(checkInterval, async () => {
    try {
  ...
  scheduledJobs.set("checkReminders", checkJob);

  const cleanupJob = cron.schedule("0 2 * * *", async () => {
    console.log("[Scheduler] Running daily cleanup of expired reminders...");
    await cleanupExpiredReminders();
  });
  scheduledJobs.set("cleanupExpiredReminders", cleanupJob);

  const recurringJob = cron.schedule("0 3 * * *", async () => {
    console.log("[Scheduler] Processing recurring reminders...");
    await processRecurringReminders();
  });
  scheduledJobs.set("processRecurringReminders", recurringJob);

  console.log(`✓ Reminder Scheduler initialized (check: "${checkInterval}", cleanup: 2 AM daily, recurring: 3 AM daily)`);
};
