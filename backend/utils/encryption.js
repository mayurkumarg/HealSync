...
// Encryption key must come from the environment (see generateEncryptionKey.js).
// It is a 64-char hex string (32 bytes) and must be decoded from hex, not used as raw UTF-8 bytes,
// or aes-256-gcm will reject it with "Invalid key length".
if (!process.env.ENCRYPTION_KEY) {
  throw new Error(
    "ENCRYPTION_KEY environment variable is not set. Run `node generateEncryptionKey.js` and add the output to your .env file. " +
    "Encrypting without a fixed key would make previously-encrypted data unreadable after every restart."
  );
}
const ENCRYPTION_KEY = Buffer.from(process.env.ENCRYPTION_KEY, "hex");
if (ENCRYPTION_KEY.length !== 32) {
  throw new Error("ENCRYPTION_KEY must be a 64-character hex string (32 bytes). Run `node generateEncryptionKey.js` to generate a valid one.");
}
